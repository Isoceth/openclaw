...
/* ── Agent name lookup (fork change: webchat session dropdown) ── */

describe("resolveSessionDisplayName – agent name lookup", () => {
  const agents = [{ id: "main", name: "Hex" }];

  it("returns agent name from agents list for main session", () => {
    expect(
      resolveSessionDisplayName("agent:main:main", row({ key: "agent:main:main" }), agents),
    ).toBe("Hex");
  });

  it("user label takes priority over agent name", () => {
    expect(
  ...
  it("ignores gateway displayName — agent name wins for main session", () => {
    expect(
      resolveSessionDisplayName(
        "agent:main:main",
