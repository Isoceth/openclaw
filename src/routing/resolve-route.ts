...
function normalizeBindingMatch(
  ...
  return {
    ...
    workspaceId: normalizeId(match?.workspaceId) || null,
...
function hasWorkspaceConstraint(match: NormalizedBindingMatch): boolean {
  return Boolean(match.workspaceId);
}
...
function matchesBindingScope(match: NormalizedBindingMatch, scope: BindingScope): boolean {
  ...
  if (match.workspaceId && match.workspaceId !== scope.workspaceId) {
    return false;
  }
  if (match.roles) {
...
export function resolveAgentRoute(input: ResolveAgentRouteInput): ResolvedAgentRoute {
  ...
  const tiers: Array<{
    ...
    {
      matchedBy: "binding.workspace",
      enabled: Boolean(workspaceId),
      scopePeer: peer,
      predicate: (candidate) => hasWorkspaceConstraint(candidate.match),
    },
    {
