...
2. **Parent peer match** (threads inherit binding from parent channel/group when no direct match).
3. **Guild match** (Discord) via `guildId`.
4. **Team match** (Slack) via `teamId`.
5. **Workspace match** (webchat) via `workspaceId`.
